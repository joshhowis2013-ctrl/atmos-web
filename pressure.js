...
const map = L.map("pressure-map", { zoomControl: true }).setView([54.2, -2.6], 5.6);
...
const els = { loading: document.getElementById("map-loading"), status: document.getElementById("map-status"), lowest: document.getElementById("lowest-pressure"), highest: document.getElementById("highest-pressure"), updated: document.getElementById("map-updated"), refresh: document.getElementById("refresh-map") };
...
async function loadPressureMap() {
  ...
  try {
    ...
    const readings = points.map((point, index) => ({ name: point[0], latitude: point[1], longitude: point[2], pressure: Array.isArray(data) ? data[index]?.current?.pressure_msl : data.current?.pressure_msl }));
    const valid = readings.filter(reading => Number.isFinite(reading.pressure));
    ...
    valid.forEach((reading, index) => {
      ...
      L.marker([reading.latitude, reading.longitude], { icon }).bindPopup(`<strong>${reading.name}</strong><br>${reading.pressure.toFixed(1)} hPa surface pressure`).addTo(markerLayer);
    });
    ...
    els.status.textContent = `${valid.length} live readings across the UK and nearby waters. Click a marker for details.`;
  } catch (error) {
